...
def test_caps_consolidated_rlc_to_six_when_diff_shows_function_boundary() -> None:
    """PR #19 r3202849011 regression: declared rlc=8 covers _gitlab() + blank + def _bitbucket."""
    # Arrange
    strategy = NovaSingleShotStrategy(
        ai_client=None,
        diff_by_file={"scripts/extract_positioning_fixtures.py": _GITLAB_DIFF},
    )
    ...
    # Assert — span capped to the actual _gitlab() body, replacement preserved verbatim
    assert result.replacement_line_count == 6
    assert len(result.code_replacement) == 8
...
def test_caps_passthrough_singleton_to_six_when_source_finding_declares_eight() -> None:
    """Singleton group hits _passthrough — validator must defend that path too."""
    # Arrange
    finding = AgentFinding(
        file_path="scripts/extract_positioning_fixtures.py",
        line_number=47,
        severity="medium",
        issue="rewrite _gitlab()",
        suggestion="see code",
        confidence=0.9,
        category="security",
        agent_name="zara",
    ...
    # Act
    result = strategy._passthrough(finding, "singleton")

    # Assert
    assert result.replacement_line_count == 6


def test_caps_source_fallback_to_six_when_nova_omits_code_replacement() -> None:
    """Nova returns no code_replacement → fall back to highest-confidence source finding,
    AND validate that source's declared rlc against the diff.
    """
    # Arrange
    zara_finding = AgentFinding(
        file_path="scripts/extract_positioning_fixtures.py",
    ...
    # Act
    result = strategy._build_consolidated(group, syn_without_code)

    # Assert — uses Zara's code_replacement but caps her wrong rlc=8 to 6
    assert result.code_replacement == zara_finding.code_replacement
    assert result.replacement_line_count == 6


# ─────────────────────────────────────────────────────────────────────────────
# Anchor-coherence — PR #20 wrong-anchor regression coverage
# ─────────────────────────────────────────────────────────────────────────────


def test_drops_code_replacement_when_def_replacement_is_anchored_mid_function_body() -> None:
    """PR #20 r3203256869: def _gitlab() suggested at line 51 (mid-body, indent 4).

    Anchor indent (4) doesn't match replacement indent (0) → drop code_replacement
    so GitHub falls back to a plain inline comment instead of a multi-line suggestion
    that would inject `def _gitlab():` into the middle of the existing function.

    Both Nova's syn and Zara's source finding carry the same def-anchored
    replacement, so both incoherence checks must fire.
    """
    # Arrange
    zara_finding = AgentFinding(
        file_path="scripts/extract_positioning_fixtures.py",
        line_number=51,
        severity="medium",
        issue="missing timeout on urlopen",
    ...
    # Assert — incoherent anchor → suggestion downgraded to prose-only
    assert result.code_replacement is None
    assert result.replacement_line_count == 1


def test_drops_code_replacement_in_passthrough_when_anchor_indent_mismatches_replacement() -> None:
    """Singleton group with wrong anchor must also drop code_replacement."""
    # Arrange
    finding = AgentFinding(
        file_path="scripts/extract_positioning_fixtures.py",
        line_number=51,
...
def test_keeps_code_replacement_when_def_replacement_is_anchored_at_def_line() -> None:
    """Coherent anchor (def at line 47 → def at line 47) is preserved unchanged."""
    # Arrange
    finding = AgentFinding(
        file_path="scripts/extract_positioning_fixtures.py",
        line_number=47,
    ...
    # Assert — coherent: keep code_replacement; rlc still capped to 6 by span validator
    assert result.code_replacement == _GITLAB_REPLACEMENT_EIGHT_LINES
    assert result.replacement_line_count == 6
