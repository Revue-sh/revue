...
def test_complete_with_tools_passes_output_config_to_loop() -> None:
    ...
    with patch("revue.core.ai_client.anthropic.Anthropic"):
        client = AnthropicClient(_config())

    with patch("revue.core.tool_loop.anthropic_tool_loop") as mock_loop:
...
def test_openrouter_client_translates_output_config_to_response_format() -> None:
    ...
    would persist.
    """
    from revue.core.ai_client import OpenRouterClient
    ...
    with patch("revue.core.tool_loop.openai_tool_loop") as mock_loop:
