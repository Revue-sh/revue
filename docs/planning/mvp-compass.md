...
| 🟠 Strongly-should | 1 | High risk to ship without; close before launch if at all possible |
...
315/357/372/374/334/325/**354/395/393/360/331/362/339** merged — all hard launch blockers cleared. The **REVUE-381 → REVUE-389** revenue go-live chain is deferred post-launch. REVUE-362 ✅ done (observability). REVUE-339 ✅ done (cooperative deadline + agent_timeout finalize). **Next priority: REVUE-361 (email deliverability) — sole remaining strongly-should.**
...
| ~~REVUE-339 + REVUE-341~~ | REVUE-339 ✅ done — collision moot; REVUE-341 can proceed solo |
...
| 2 | ✅ **REVUE-339** (cooperative deadline + agent_timeout finalize) — merged 2026-06-04 | `revue_core` agent_timeout config | — done |
| 3 | ✅ **REVUE-397** (licence-validator retry on transient) — merged 2026-06-04 | `revue_core/.../license_validator.py` | — done |
...
- ~~**REVUE-339 + REVUE-341**~~ — REVUE-339 is ✅ done; REVUE-341 can proceed solo (no collision partner remaining).

**Net parallelism (as of 2026-06-04 — all hard blockers cleared; REVUE-331 + REVUE-362 + REVUE-339 ✅ done):** zero hard launch blockers remain. One strongly-should remains: **REVUE-361 (email deliverability)** — independent of all open tickets. Cleanest concurrent work: **REVUE-361** — sole remaining strongly-should; **REVUE-397 ✅ done** (merged 2026-06-04). REVUE-341 (per-surface agent_timeout defaults) can now proceed solo — the REVUE-339 + REVUE-341 same-file collision caveat is moot (339 done). The **REVUE-381 → 389** revenue chain stays **deferred to post-launch** (pre-revenue sole-trader launch + 1st Formations KYC rejection).

Worktree creation:

```bash
# REVUE-315/325/354/395/393/360/362/339 ✅ DONE. Revenue go-live = REVUE-389 (after REVUE-381 entity reg, externally blocked).
git worktree add ../revue-361 REVUE-361-email-deliverability      # src/web/ (verify non-overlap with 362 first)
# REVUE-397 ✅ done — merged 2026-06-04 (licence-validator retry on transient)
...
| Jira | Story | Shipped |
|------|-------|---------|
| REVUE-397 | Licence-validator retry transient API-unreachable before failing — bounded hardcoded retry + backoff, definitive outcomes never retried, SRP _ValidationResult + _compute_backoff refactor (HIGH tooling follow-up from PR #210 transient pipeline failure; relates 278/371) | 2026-06-04 |
| REVUE-339 | Cooperative deadline + finalize budget reservation for agent_timeout — shared global deadline, finalize HTTP bounded by remaining budget so mid-synthesis findings aren't lost on hard-kill | 2026-06-04 |
