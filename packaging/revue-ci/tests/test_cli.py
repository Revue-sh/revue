...
from revue_ci.cli import build_parser, cmd_init, cmd_review, cmd_validate
...
@patch("revue_ci.cli.parse_diff_file")
...
def test_review_calls_ai_client(tmp_path, capsys):
    ...
    with patch.dict(os.environ, {"ANTHROPIC_API_KEY": "test-key-123"}), \
         patch("revue_core.core.pipeline.validate_license", return_value=_stub_license_info()), \
...
@patch("revue_core.core.pipeline.parse_diff_file")
...
def test_resolve_pr_id_from_env_bitbucket(monkeypatch):
    ...
    from revue_ci.cli import _resolve_pr_id_from_env
...
def test_cli_pr_description_file_read(tmp_path, capsys):
    ...
    import textwrap
    from unittest.mock import MagicMock, patch
    from revue_ci.cli import cmd_review, build_parser
    ...
    with patch("revue_ci.cli.load_config", return_value=mock_cfg), \
         patch("revue_ci.cli.validate_config", return_value=[]), \
         patch("revue_ci.cli.ReviewPipeline", return_value=mock_pipeline):
        cmd_review(args)
...
from revue_ci.cli import _format_finding
...
def test_format_finding_shows_agent_name_for_all_builtin_agents() -> None:
    ...
    from revue_ci.cli import _AGENT_DISPLAY_NAMES, _CATEGORY_MAP
    cases = [
...
def test_print_metrics_summary_no_collector() -> None:
    ...
    from revue_ci.cli import _print_metrics_summary
...
def _cmd_review_with_posting_result(posting_result, tmp_path) -> int:
    ...
    with patch("revue_ci.cli.load_config", return_value=mock_cfg), \
         patch("revue_ci.cli.validate_config", return_value=[]), \
         patch("revue_ci.cli.ReviewPipeline", return_value=mock_pipeline), \
         patch("revue_ci.cli._post_to_bitbucket", return_value=posting_result):
