...
from revue_core.core.ai_client import _CACHE_CONTROL_1H, CompletionResult, TokenUsage
...
def test_load_custom_agents_nonexistent_path():
    ...
    # Arrange / Act
    with patch("revue_core.core.agent_loader.Log") as mock_log:
...
def test_agent_runner_marks_failed_on_http_error():
    ...
    from revue_core.core.agent_runner import run_agents_parallel, AgentProtocol
...
from revue_core.core.pattern_injection import build_pattern_prompt_sections, inject_patterns
...
class TestInfrastructureAgentsRegistered:
    ...
    def test_every_infrastructure_agent_has_non_empty_system_prompt(self) -> None:
        ...
        from revue_core.core.cleo_router import _INFRASTRUCTURE_AGENTS
...
class TestLoadedAgentToolUse:
    ...
    def test_load_agents_from_dir_threads_read_file_tool(self):
        ...
        agents = load_agents_from_dir(
            Path(revue_core.__file__).parent / "agents",
            _mock_client(),
            max_tokens=4096,
            read_file_tool=tool,
        )
