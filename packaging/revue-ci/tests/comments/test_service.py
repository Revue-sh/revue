...
from revue_core.core.dedup_consolidator import NovaConsolidator, _parse_thread_decisions
...
def test_analyse_reply_threads_splits_large_list_into_batches() -> None:
    ...
    from revue_core.core.dedup_consolidator import _THREAD_BATCH_SIZE
...
def test_wont_fix_service_updates_state_to_wont_fix(tmp_path) -> None:
    ...
    mock_adapter = MagicMock()
    with patch("revue_core.comments.service.WontFixReplyService._ensure_lessons_pr",
...
def test_wont_fix_service_lessons_pr_failure_posts_yaml_block(tmp_path) -> None:
    ...
    mock_adapter = MagicMock()
    with patch(
             "revue_core.comments.service.WontFixReplyService._ensure_lessons_pr",
...
def test_respond_posts_replies_and_creates_lessons_pr(tmp_path) -> None:
    ...
    mock_adapter = MagicMock()
    with patch("revue_core.comments.service.WontFixReplyService._append_pattern_to_config") as mock_append, \
         patch("revue_core.comments.service.WontFixReplyService._ensure_lessons_pr",
...
def test_classify_skips_ai_for_sentinel_last_threads(tmp_path) -> None:
    ...
    with patch("revue_core.core.dedup_consolidator.NovaConsolidator.analyse_reply_threads") as mock_ai_call:
...
def test_respond_processes_thread_when_dev_reply_after_sentinel(tmp_path) -> None:
    ...
    with patch("revue_core.comments.service.WontFixReplyService._append_pattern_to_config"):
        svc = WontFixReplyService(
            repo_path=str(tmp_path),
            ai_client=MagicMock(),
            repo_owner="owner",
...
def test_sentinel_map_contains_exactly_four_sentinels() -> None:
    ...
    from revue_core.comments.service import SentinelKind, _SENTINEL_MAP
...
def test_sentinel_kind_composite_reply_returns_first_match() -> None:
    ...
    from revue_core.comments.service import SentinelKind, get_sentinel_kind
...
def test_sentinel_kind_none_for_plain_text() -> None:
    ...
    from revue_core.comments.service import get_sentinel_kind
...
def test_sentinel_is_last_true_for_any_sentinel_kind() -> None:
    ...
    from revue_core.comments.service import _sentinel_is_last
...
def test_terminal_is_last_only_for_terminal_kind() -> None:
    ...
    from revue_core.comments.service import _terminal_is_last
